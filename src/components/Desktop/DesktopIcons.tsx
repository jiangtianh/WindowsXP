...
const DesktopIcons: React.FC = () => {
    ...
    return (
        <div className="inset-0 h-full w-full overflow-hidden"
            style={{ paddingTop: `${paddingTop}px`, paddingLeft: `${paddingX}px`, paddingRight: `${paddingX}px` }}
        >
            {Object.entries(desktopIcons).map(([key, icon]) => (
                <DesktopIcon
                    key={key}
                    windowKey={key as any}
                    iconInfo={icon}
                    gridCellSize={{ width: GRID_HEIGHT, height: GRID_WIDTH }}
                />
            ))}
            {showGrid && (
